...
class ProductCreateTestCaes(APITestCase):
...
class ProductUpdateTestCase(APITestCase):
    ...
    def test_upload_product_photo(self):
        product = Product.objects.first()
        original_photo = product.original_photo
        ...
        self.assertEqual(response.data['photo'], original_photo)
        try:
            updated = Product.objects.get(id=product_id)
            expected_photo_path = os.path.join(
                settings.MEDIA_ROOT, 'products', 'vitamin-iron'
            )
            self.aeertTrue(
                updated.photo.path.startswith(expected_photo_path)
            )
        finally:
